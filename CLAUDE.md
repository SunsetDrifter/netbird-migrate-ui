...
NetBird Migration UI — a Next.js web application for migrating configurations between NetBird accounts. Users connect to a source and destination NetBird instance, select resources to migrate, resolve conflicts, and execute the migration with real-time progress streaming.
...
1. **`app/page.tsx`** — Connect source + destination (validates with test API call)
...
5. Add to `defaultSelection` in `hooks/use-migration-state.ts`
6. Add auto-select logic and `<ResourceList>` in `app/migrate/page.tsx`
